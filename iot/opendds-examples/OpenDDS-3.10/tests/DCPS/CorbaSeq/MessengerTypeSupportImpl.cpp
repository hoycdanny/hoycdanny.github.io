...
void gen_find_size(const Messenger::Message& stru, size_t& size, size_t& padding)
{
  ...
  gen_find_size(stru.outside_long_seq, size, padding);
}
...
struct MetaStructImpl<Messenger::Message> : MetaStruct {
  ...
  Value getValue(Serializer& ser, const char* field) const
  {
    ...
    gen_skip_over(ser, static_cast<Messenger2::LongSeq*>(0));
    if (!field[0]) {
      return 0;
    }
  ...
  {
    static const char* names[] = {"from", "subject", "subject_id", "text", "count", "bool_seq", "longdouble_seq", "short_seq", "ushort_seq", "char_seq", "longlong_seq", "string_seq", "wchar_seq", "double_seq", "long_seq", "ulonglong_seq", "wstring_seq", "float_seq", "octet_seq", "ulong_seq", "outside_long_seq", 0};
    return names;
  }
  ...
  const void* getRawField(const void* stru, const char* field) const
  {
    ...
    if (std::strcmp(field, "outside_long_seq") == 0) {
      return &static_cast<const T*>(stru)->outside_long_seq;
    }
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct Messenger::Message)");
  }
    ...
    const char* rhsFieldSpec, const MetaStruct& rhsMeta) const
  {
    ...
    if (std::strcmp(field, "outside_long_seq") == 0) {
      static_cast<T*>(lhs)->outside_long_seq = *static_cast<const Messenger2::LongSeq*>(rhsMeta.getRawField(rhs, rhsFieldSpec));
      return;
    }
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct Messenger::Message)");
  }
