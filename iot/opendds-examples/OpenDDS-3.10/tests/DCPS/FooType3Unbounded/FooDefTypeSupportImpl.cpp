...
void gen_find_size(const Xyz::Foo& stru, size_t& size, size_t& padding)
{
  ...
  gen_find_size(stru.unbounded_data, size, padding);
}
...
struct MetaStructImpl<Xyz::Foo> : MetaStruct {
  ...
  Value getValue(Serializer& ser, const char* field) const
  {
    ...
    gen_skip_over(ser, static_cast<Xyz::UnboundedData*>(0));
    if (!field[0]) {
      return 0;
    }
  ...
  {
    static const char* names[] = {"a_long_value", "handle_value", "sample_sequence", "writer_id", "unbounded_data", 0};
    return names;
  }
  ...
  const void* getRawField(const void* stru, const char* field) const
  {
    ...
    if (std::strcmp(field, "unbounded_data") == 0) {
      return &static_cast<const T*>(stru)->unbounded_data;
    }
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct Xyz::Foo)");
  }
    ...
    const char* rhsFieldSpec, const MetaStruct& rhsMeta) const
  {
    ...
    if (std::strcmp(field, "unbounded_data") == 0) {
      static_cast<T*>(lhs)->unbounded_data = *static_cast<const Xyz::UnboundedData*>(rhsMeta.getRawField(rhs, rhsFieldSpec));
      return;
    }
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct Xyz::Foo)");
  }
