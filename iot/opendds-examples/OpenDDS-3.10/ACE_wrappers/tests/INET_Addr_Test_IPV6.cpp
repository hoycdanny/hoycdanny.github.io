// $Id: INET_Addr_Test_IPV6.cpp 2772 2015-11-19 15:07:45Z huangh $
  ...
  if (ACE::ipv6_enabled ())
    {
     ...
     if (getifaddrs(&ifaddr) == -1) 
       {
         ACE_ERROR ((LM_ERROR,
                     ACE_TEXT ("getifaddrs failed.\n")));
         status = 1;
       }
     else 
       {

         /* Walk through linked list, find the first non-loopback AF_INET6 interface */
         for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) 
           {
             ...
             if (-1 == link_local_addr.set_interface (ifa->ifa_name))
               {
                 ACE_ERROR ((LM_ERROR,
                             ACE_TEXT ("%p\n"),
