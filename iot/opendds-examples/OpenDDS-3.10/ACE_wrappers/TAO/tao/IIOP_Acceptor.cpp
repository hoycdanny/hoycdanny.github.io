// $Id: IIOP_Acceptor.cpp 2647 2015-08-28 15:54:28Z mesnierp $
...
TAO_IIOP_Acceptor::TAO_IIOP_Acceptor (void)
  : TAO_Acceptor (IOP::TAG_INTERNET_IOP),
    ...
    interfaces_probed_ (false),
    probe_def_type_ (AF_UNSPEC),
    base_acceptor_ (this),
...
bool
TAO_IIOP_Acceptor::refresh (TAO_MProfile &mprof)
{
  int defport = this->default_address_.get_port_number ();

  if (!this->interfaces_probed_ || this->hostname_in_ior_ != 0)
    {
      return false;
    }
  mprof.rewind ();
  this->probe_interfaces ();
  CORBA::ULong ndx = 0;
  bool updated = false;
  bool shared = this->orb_core_->orb_params ()->shared_profile () != 0;
  ACE_Auto_Ptr<TAO::ObjectKey> okey;

  for (TAO_Profile *p = mprof.get_next (); p != 0; p = mprof.get_next ())
    {
      if (p->tag () != this->tag ())
        {
          continue;
      ...
      updated = true;
      TAO_IIOP_Profile *iprof = dynamic_cast<TAO_IIOP_Profile *> (p);
      if (shared)
        {
          TAO_Endpoint *ep = p->base_endpoint ();
          while (ep != 0 || ndx < this->endpoint_count_)
            {
              if (ep != 0)
                {
                  TAO_IIOP_Endpoint *iep = dynamic_cast<TAO_IIOP_Endpoint *> (ep);
                  TAO_Endpoint *next = ep->next ();
                  if (iep->port () == defport)
                    {
                      if (ndx == this->endpoint_count_)
                        {
                          iprof->remove_endpoint (iep);
                        }
                      else
                        {
                           iep->host (this->hosts_[ndx++]);
                        }
                    }
                  ep = next;
                }
              else
                {
                  TAO_IIOP_Endpoint *endpoint = 0;
                  ACE_NEW_RETURN (endpoint,
                                  TAO_IIOP_Endpoint (this->hosts_[ndx],
                                                     defport,
                                                     this->addrs_[ndx]),
                                  false);
                  iprof->add_endpoint (endpoint);
                  ndx++;
                }
            }
          return updated;
        }
      TAO_IIOP_Endpoint *iep = dynamic_cast<TAO_IIOP_Endpoint *> (p->base_endpoint ());
      if (okey.get () == 0)
        {
          okey.reset (p->_key ());
        }
      if (iep->port () == this->addrs_[0].get_port_number ())
        {
          if (ndx < this->endpoint_count_)
            {
              iep->host (this->hosts_[ndx++]);
  ...
  if (ndx < this->endpoint_count_)
    {
      mprof.grow (this->endpoint_count_ - ndx);
    }

  for (; ndx < this->endpoint_count_; ++ndx)
    {
      TAO_IIOP_Profile *pfile = 0;
      ACE_NEW_RETURN (pfile,
                      TAO_IIOP_Profile (this->hosts_[ndx],
                                        defport,
                                        *okey,
                                        this->addrs_[ndx],
                                        this->version_,
  ...
  if (specified_hostname.length() == 0)
    {
      ...
      this->probe_def_type_ = def_type;
      if (this->probe_interfaces () == -1)
        return -1;
                                  ...
                                  int *def_type)
{
  ...
  if (!ipv6_in_host && !host_defaulted)
    {
      ...
      if (TAO_IIOP_Endpoint::try_ipv4_first_)
        {
          res_ipv4 = addr.set((unsigned short)0, tmp_host, 1, AF_INET);
        }
      if (res_ipv4 == -1 &&
          addr.set((unsigned short)0, tmp_host) != 0)
        {
          return -1;
        }
  ...
  if (host_defaulted)
  ...
  else if (port_separator_loc == 0)
    {
      ...
      if (TAO_IIOP_Endpoint::try_ipv4_first_)
        {
          res_ipv4 = addr.set((unsigned short)0,
                              specified_hostname.c_str(), 1, AF_INET);
        }
      if (res_ipv4 == -1 &&
          addr.set((unsigned short)0,
                   specified_hostname.c_str()) != 0)
        {
          return -1;
        }
  ...
  else
    {
      ...
      if (TAO_IIOP_Endpoint::try_ipv4_first_)
        {
          res_ipv4 = addr.set (address, AF_INET);
        }
      if (res_ipv4 == -1 && addr.set (address) != 0)
        {
          return -1;
        }
      if (tmp_host[0] == '\0')
...
int
TAO_IIOP_Acceptor::probe_interfaces (void)
{
  ...
  if (this->interfaces_probed_)
    {
      delete [] this->addrs_;
      for (CORBA::ULong i = 0; i < this->endpoint_count_; ++i)
        CORBA::string_free (this->hosts_[i]);
      delete [] this->hosts_;
      this->endpoint_count_ = 0;
    }
  this->interfaces_probed_ = true;
  ...
  for (size_t j = 0; j < if_cnt; ++j)
    ...
    else if (!this->orb_core_->orb_params ()->use_ipv6_link_local () &&
...
#if defined (ACE_HAS_IPV6)
  bool ipv4_only = this->probe_def_type_ == AF_INET;
  bool ipv6_only = (this->probe_def_type_ == AF_INET6) ||
    this->orb_core_->orb_params ()->connect_ipv6_only ();
  ...
  for (size_t i = 0; i < if_cnt; ++i)
    {
      ...
      if (!this->orb_core_->orb_params ()->use_ipv6_link_local () &&
      ...
      if (this->hostname (this->orb_core_,
                          if_addrs[i],
