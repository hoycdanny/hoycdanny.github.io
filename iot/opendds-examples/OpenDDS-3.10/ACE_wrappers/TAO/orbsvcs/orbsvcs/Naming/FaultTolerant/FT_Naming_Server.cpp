 ...
 *
 * $Id: FT_Naming_Server.cpp 2861 2016-06-12 04:24:28Z mesnierp $
 *
...
TAO_FTNS_Notifier::TAO_FTNS_Notifier (TAO_FT_Naming_Server &owner)
  : owner_ (owner)
{
}
...
{
  if (this->iors_pending_)
    {
      if (this->iors_.combined)
        {
          this->owner_.update_ior (FT_Naming::ctx_name, this->iors_.root_context.in());
          this->owner_.update_ior (FT_Naming::obj_grp, this->iors_.naming_manager.in());
        }
      else
        {
          this->owner_.combine_iors (FT_Naming::ctx_name, this->iors_.root_context.in());
          this->owner_.combine_iors (FT_Naming::obj_grp, this->iors_.naming_manager.in());
          this->owner_.replicator_->send_registration (true);
        }

      this->iors_pending_ = false;
    }

  for (CORBA::ULong i = 0; i < this->infos_.length(); i++)
    {
      if (infos_[i]._d() == FT_Naming::ctx_name)
        {
          this->owner_.update_naming_context (infos_[i].ctx());
        }
      else
        {
          this->owner_.update_object_group (infos_[i].grp());
        }
    }
  infos_.length(0);
  return 0;
...
void
TAO_FT_Naming_Server::update_info (const FT_Naming::UpdateInfoSeq &infos)
{
  CORBA::ULong len = this->notifier_.infos_.length ();
  this->notifier_.infos_.length (len + infos.length());
  for (CORBA::ULong i = 0; i < infos.length(); i++)
    {
      this->notifier_.infos_[len+i] = infos[i];
    }
  this->orb_->orb_core ()->reactor ()->notify (&this->notifier_);
}

void
TAO_FT_Naming_Server::update_iors ( const ::FT_Naming::ReplicaInfo & iors )
{
  this->notifier_.iors_.combined = iors.combined;
  this->notifier_.iors_.root_context = iors.root_context;
  this->notifier_.iors_.naming_manager = iors.naming_manager;
  this->notifier_.iors_pending_ = true;
  this->orb_->orb_core ()->reactor ()->notify (&this->notifier_);
}
...
TAO_FT_Naming_Server::init_with_orb (int argc,
                                     ...
                                     CORBA::ORB_ptr orb)
{
  ACE_NEW_RETURN (this->replicator_, FT_Naming_Replication_Manager (*this), -1);
